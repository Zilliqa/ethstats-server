...
{
	if( !_.isUndefined(stats) && !_.isUndefined(stats.pending) && !_.isUndefined(stats.queued))
	{
		if(!_.isEqual(stats.pending, this.stats.pending) || !_.isEqual(stats.queued, this.stats.queued))
		{
			this.stats.pending = stats.pending;
			this.stats.queued = stats.queued;
...
Node.prototype.setBasicStats = function(stats, callback)
{
	if( !_.isUndefined(stats) )
	{
		...
		}) )
		{
			...
			this.stats.currentView = (!_.isUndefined(stats.currentView) ? stats.currentView : 0);
			this.stats.nextViewChange = (!_.isUndefined(stats.nextViewChange) ? stats.nextViewChange : 0);
			this.stats.highQcView = (!_.isUndefined(stats.highQcView) ? stats.highQcView : 0);
			this.stats.gasPrice = stats.gasPrice;
			this.stats.uptime = stats.uptime;
			if( !_.isUndefined(stats.pending) )
				this.stats.pending = stats.pending;
			if( !_.isUndefined(stats.queued) )
				this.stats.queued = stats.queued;
